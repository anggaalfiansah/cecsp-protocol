...
import { decryptAES, getHeadersToken, encryptAES, makeToken } from "../utils/crypto";
...
export async function httpSecure(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (req.body?.cipher && typeof req.body.cipher === "string") {
      try {
        const decryptedStr = decryptAES(req.body.cipher);
    ...
    res.json = function (body: any): Response {
      ...
      const encryptedBody = encryptAES(plainString);
